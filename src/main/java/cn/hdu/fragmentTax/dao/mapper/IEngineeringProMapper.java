...
@Insert("INSERT INTO `engineering_pro`(`id`, `stu_id`, `pro_name`, `pro_intr`, `performance`, `work`, `ranking`, `total_number`, `pro_state`, `enterprise_name`, `enterprise_teacher`, `teacher`, `proof_material_id`, `score`, `state`, `created_time`) VALUES(#{id}, #{stuId}, #{proName}, #{proIntr}, #{performance}, #{work}, #{ranking}, #{totalNumber}, #{proState}, #{enterpriseName}, #{enterpriseTeacher}, #{teacher}, #{proofMaterialId}, #{score}, #{state}, #{createdTime})")
void insert(EngineeringProEntity engineering_proEntity);

@Update("UPDATE `engineering_pro` SET id=#{id}, stu_id=#{stuId}, pro_name=#{proName}, pro_intr=#{proIntr}, performance=#{performance}, work=#{work}, ranking=#{ranking}, total_number=#{totalNumber}, pro_state=#{proState}, enterprise_name=#{enterpriseName}, enterprise_teacher=#{enterpriseTeacher}, teacher=#{teacher}, proof_material_id=#{proofMaterialId}, score=#{score}, state=#{state}, created_time=#{createdTime} WHERE `id` = #{id}")
