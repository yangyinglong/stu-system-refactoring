...
@Insert("INSERT INTO `academic_exchange`(`id`, `stu_id`, `exchange_type`, `organization`, `conference_name`, `start_time`, `end_time`, `level`, `result`, `proof_material_id`, `score`, `state`) VALUES(#{id}, #{stuId}, #{exchangeType}, #{organization}, #{conferenceName}, #{startTime}, #{endTime}, #{level}, #{result}, #{proofMaterialId}, #{score}, #{state})")
void insert(AcademicExchangeEntity academic_exchangeEntity);

@Update("UPDATE `academic_exchange` SET id=#{id}, stu_id=#{stuId}, exchange_type=#{exchangeType}, organization=#{organization}, conference_name=#{conferenceName}, start_time=#{startTime}, end_time=#{endTime}, level=#{level}, result=#{result}, proof_material_id=#{proofMaterialId}, score=#{score}, state=#{state} WHERE `id` = #{id}")
