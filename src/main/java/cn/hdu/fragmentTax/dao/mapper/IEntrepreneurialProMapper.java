...
public interface IEntrepreneurialProMapper {
    ...
    @Update("UPDATE `entrepreneurial_pro` SET state=0 WHERE `id` = #{id}")
    void delete(@Param("id") Integer id);

    @Select("SELECT `id`, `stu_id`, `pro_type`, `pro_name`, `ranking`, `total_number`, `pro_state`, `pro_level`, `pro_result`, `score`, `teacher`, `proof_material_id`, `state`, `created_time` FROM `entrepreneurial_pro` WHERE `stu_id` = #{stuId} and `state` <> 0")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "stuId", column = "stu_id"),
            @Result(property = "proType", column = "pro_type"),
