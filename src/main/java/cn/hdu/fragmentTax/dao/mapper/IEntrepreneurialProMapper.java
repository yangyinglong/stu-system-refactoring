...
@Insert("INSERT INTO `entrepreneurial_pro`(`id`, `stu_id`, `pro_type`, `pro_name`, `ranking`, `total_number`, `pro_state`, `pro_level`, `pro_result`, `score`, `teacher`, `proof_material_id`, `state`, `created_time`) VALUES(#{id}, #{stuId}, #{proType}, #{proName}, #{ranking}, #{totalNumber}, #{proState}, #{proLevel}, #{proResult}, #{score}, #{teacher}, #{proofMaterialId}, #{state}, #{createdTime})")
void insert(EntrepreneurialProEntity entrepreneurial_proEntity);

@Update("UPDATE `entrepreneurial_pro` SET id=#{id}, stu_id=#{stuId}, pro_type=#{proType}, pro_name=#{proName}, ranking=#{ranking}, total_number=#{totalNumber}, pro_state=#{proState}, pro_level=#{proLevel}, pro_result=#{proResult}, score=#{score}, teacher=#{teacher}, proof_material_id=#{proofMaterialId}, state=#{state}, created_time=#{createdTime} WHERE `id` = #{id}")
