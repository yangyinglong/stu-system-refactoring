...
@Insert("INSERT INTO `work`(`id`, `stu_id`, `company_name`, `company_type`, `work_type`, `score`, `state`, `created_time`) VALUES(#{id}, #{stuId}, #{companyName}, #{companyType}, #{workType}, #{score}, #{state}, #{createdTime})")
void insert(WorkEntity workEntity);

@Update("UPDATE `work` SET id=#{id}, stu_id=#{stuId}, company_name=#{companyName}, company_type=#{companyType}, work_type=#{workType}, score=#{score}, state=#{state}, created_time=#{createdTime} WHERE `id` = #{id}")
